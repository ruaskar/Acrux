...
| **LLM summaries** — `keymd summarize` (opt-in; your own model) | ✅ caches a prose summary per gated file via **your** endpoint+key (OpenAI- or Anthropic-protocol, incl. local models), sha-incremental, secret-redacted; served as the `summary:` lead in `.key.md` + gate + graph. First pass ≈ one scan; wins on reuse. |
